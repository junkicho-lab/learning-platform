...
def build_chapters(source_dir, output_file, var_name="CHAPTERS_DATA"):
    """챕터 데이터 빌드 함수"""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_dir = os.path.dirname(script_dir)  # 프로젝트 루트 디렉토리
    md_dir = os.path.join(project_dir, source_dir)
    ...
    # JavaScript 파일 생성
    js_output = f"// 자동 생성된 챕터 데이터 ({source_dir} 마크다운에서 변환)\n"
    js_output += "// 생성일: " + datetime.now().strftime('%Y-%m-%d %H:%M:%S') + "\n"
    js_output += f"const {var_name} = [\n"
...
"""
    ...
    # 파일 저장
    js_path = os.path.join(project_dir, output_file)
    
    with open(js_path, 'w', encoding='utf-8') as f:
        f.write(js_output)
    
    print(f"\n✅ 챕터 데이터 생성 완료: {output_file}")
...
def main():
    ...
    # 기본값
    source_dir = '수정본'
    output_file = 'js/chapters.js'
    var_name = 'CHAPTERS_DATA'
    ...
    build_chapters(source_dir, output_file, var_name)

#!/usr/bin/env python3
"""
마크다운 파일들을 합쳐서 출판용 파일(EPUB, PDF, DOCX)로 변환하는 스크립트
"""
import os
import subprocess
import unicodedata
...
def convert_to_formats(merged_md, output_dir, base_name):
    ...
    try:
        ...
        print(f"  PDF Error (xelatex required): {e.stderr.decode()[:200]}")
...
def main():
    ...
    # 수정본 폴더 사용 (원본 버전)
    source_dir = base_dir / '수정본'
    output_dir = base_dir / '출판물(2판)'
    output_dir.mkdir(exist_ok=True)
    
    print("=== 원본 버전 변환 ===")
    print("\n1. 마크다운 파일 병합 중...")
    merged_md = output_dir / '옵시디언으로_교실_기록하기_원본.md'
    merge_markdown_files(source_dir, merged_md)
    
    print("\n2. 출판 형식으로 변환 중...")
    convert_to_formats(merged_md, output_dir, '옵시디언으로_교실_기록하기_원본')
    
    # 축약본 폴더 사용
    source_dir_abridged = base_dir / '수정본_축약'
    
    print("\n=== 축약본 버전 변환 ===")
    print("\n1. 마크다운 파일 병합 중...")
    merged_md_abridged = output_dir / '옵시디언으로_교실_기록하기_축약본.md'
    merge_markdown_files(source_dir_abridged, merged_md_abridged)
    
    print("\n2. 출판 형식으로 변환 중...")
    convert_to_formats(merged_md_abridged, output_dir, '옵시디언으로_교실_기록하기_축약본')
